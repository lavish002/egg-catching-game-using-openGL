...
void drawString1(void* font, const char* s, float x, float y)
{
    glRasterPos2f(x, y);
    glColor3f(1, 1, 1);
...
void start_screen(int i, int j)
{
    ...
    char cat[9] = "Catching";
    char gam[5] = "Game";
    char made[30] = "Made by: Chandan Taneja";
    char name1[25] = "Armaan Singh Rai";
    char name2[12] = "Lavish Garg";


    glColor3f(0, 1, 0); //Egg text color
    glRasterPos2i(180, 320);
    for (k = 0; k < 4; k++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, egg[k]);

    glColor3f(1, 1, 1); // Catching text color
    glRasterPos2i(230, 320);
    for (k = 0; k < 9; k++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, cat[k]);

    glColor3f(1, 0, 0); // Game text color
    glRasterPos2i(315, 320);
    for (k = 0; k < 5; k++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, gam[k]);

    glColor3f(1, 1, 0);// Made by text color
...
void sun()
{
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(500 + 80 * cos(theta) / 2, 600 + 80 * sin(theta) / 2); // h+rcosQ , k+rsinQ
    }
    glEnd();
    glFlush();
}
...
void score()  //Counts score and beep
{
...
void display(void)
{
    ...
    if (s >= 1) // loop runs if s=1 start position
    {
        glColor3f(0, 0, 1);
        glRasterPos2i(10, 10);
        for (i = 0; i < 12; i++)
        ...
        //Egg y-cordinate decrement according to the speed
        if (egg_yc <= 10)
            egg_start();
        egg();
...
void basket_set(int a, int b)
{
    basket_x = a;
    basket_y = b;
    glutPostRedisplay(); // frsh the display
}
...
int main(int argc, char** argv)
{
    ...
    //Keys functioning activate
    glutKeyboardFunc(keys);
    //Mouse motion activate
    glutPassiveMotionFunc(basket_set);
    //Idle screen movement
